...
def generate_ward_wise_critical_report(df):
    ...
    for ward, group in critical_df.groupby("ward"):
        ...
        for idx, (_, row) in enumerate(group.iterrows(), 1):
            ...
            pdf.set_text_color(148, 163, 184)
            pdf.cell(40, 6, str("Adaptive Threshold:"), 0, 0)
            pdf.set_text_color(0, 212, 255)
            pdf.cell(0, 6, str(f"{adaptive_thr:.3f}"), 0, 1)
            # ─────────────────────────────────────────────────────────────────

            pdf.set_text_color(148, 163, 184)
            pdf.cell(40, 6, str("Components:"), 0, 0)
            pdf.set_text_color(255, 59, 92)
            pdf.cell(35, 6, str(f"CVE:{row.get('static', 0):.3f}"), 0, 0)
            pdf.set_text_color(255, 140, 66)
            pdf.cell(35, 6, str(f"Beh:{row.get('beh', 0):.3f}"), 0, 0)
            pdf.set_text_color(0, 212, 255)
            pdf.cell(35, 6, str(f"GNN:{row.get('gnn', 0):.3f}"), 0, 0)
            pdf.set_text_color(245, 200, 66)
            pdf.cell(0,  6, str(f"XGB:{row.get('xgb', 0):.3f}"), 0, 1)

            pdf.set_text_color(148, 163, 184)
            pdf.cell(40, 6, str("Action Required:"), 0, 0)
            pdf.set_text_color(255, 59, 92)
            pdf.multi_cell(0, 6, str(action), 0, 1)

            pdf.set_font("helvetica", "I", 8)
            pdf.set_text_color(100, 116, 139)
            pdf.multi_cell(0, 5, str(f"Note: {rule['note']}"), 0, 1)
            pdf.ln(4)
    ...
    pdf.set_font("helvetica", "B", 14)
    pdf.set_text_color(226, 232, 240)
    pdf.cell(0, 10, str("COMPLIANCE AND RECOMMENDATIONS"), 0, 1)
    pdf.set_draw_color(255, 59, 92)
    pdf.line(10, pdf.get_y(), 200, pdf.get_y())
    pdf.ln(6)
    ...
    pdf.set_font("helvetica", "B", 11)
    pdf.set_text_color(255, 59, 92)
    pdf.cell(0, 8, str("Immediate Actions Required - BY WARD:"), 0, 1)
    pdf.set_font("helvetica", "", 10)
    pdf.set_text_color(148, 163, 184)
    for ward, group in critical_df.groupby("ward"):
        pdf.cell(0, 6, str(
    ...
    pdf.set_font("helvetica", "B", 11)
    pdf.set_text_color(255, 59, 92)
    pdf.cell(0, 8, str("Ward Supervisor Responsibilities:"), 0, 1)
    pdf.set_font("helvetica", "", 10)
    pdf.set_text_color(148, 163, 184)
    pdf.multi_cell(0, 6, str(
    ...
    pdf.set_font("helvetica", "B", 11)
    pdf.set_text_color(255, 59, 92)
    pdf.cell(0, 8, str("Compliance Requirements:"), 0, 1)
    pdf.set_font("helvetica", "", 10)
    pdf.set_text_color(148, 163, 184)
    pdf.multi_cell(0, 6, str(
