...
FROM_NUMBER    = "+"
TO_NUMBER      = "+"
EMAIL_SENDER   = "manisharamesh2710@gmail.com"
EMAIL_PASSWORD = "kvzk atgd lcnk iuve"
...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 10, 'Clinical Guardian - Critical Devices Report', 0, 1, 'C')
        self.set_font('Arial', '', 8)
        self.cell(0, 5, f'Generated: {get_ist_now().strftime("%Y-%m-%d %H:%M:%S IST")}', 0, 1, 'C')
        self.ln(5)
...
def generate_ward_wise_critical_report(df):
    """Generate PDF report for CRITICAL devices grouped by ward"""
    ...
    # Group by ward
    for ward in sorted(critical_df["ward"].unique()):
        ward_devices = critical_df[critical_df["ward"] == ward]

        pdf.set_font("Arial", "B", 11)
        pdf.cell(0, 8, f"🏥 {ward} ({len(ward_devices)} devices)", 0, 1, "L")
        pdf.set_font("Arial", "B", 9)
        pdf.cell(60, 6, "Device Name", 1, 0, "C")
        pdf.cell(40, 6, "ID", 1, 0, "C")
        pdf.cell(40, 6, "Score", 1, 0, "C")
        pdf.cell(50, 6, "Recommended Action", 1, 1, "C")

        pdf.set_font("Arial", "", 9)
        for _, row in ward_devices.iterrows():
            rule = get_device_rule(row.get("type", ""))
            action = rule["action_critical"]

            pdf.cell(60, 6, str(row.get("name", "N/A"))[:25], 1, 0)
            pdf.cell(40, 6, str(row.get("id", "N/A"))[:15], 1, 0)
            pdf.cell(40, 6, f"{row.get('score', 0):.3f}", 1, 0)
            pdf.cell(50, 6, action[:40], 1, 1)
...
def send_bulk_email_alert_PDF(to_email, df):
    """Send PDF report of critical devices via email"""
    try:
        ...
        msg["From"] = sender
        msg["To"] = to_email
        msg["Subject"] = f"Clinical Guardian Critical Alert Report - {get_ist_now().strftime('%Y-%m-%d %H:%M')}"

        body = f"""
        Clinical Guardian - Critical Device Alert Report

        Generated: {get_ist_now().strftime('%Y-%m-%d %H:%M:%S IST')}
        Total Critical Devices: {len(critical_df)}

        This report contains all CRITICAL devices grouped by ward.
        Immediate action is required for these devices.
        ...
        # Attach PDF
        attachment = MIMEText(pdf_buffer.getvalue(), 'pdf')
        attachment.add_header('Content-Disposition', 'attachment',
                            filename=f'critical_devices_{get_ist_now().strftime("%Y%m%d_%H%M")}.pdf')
        ...
        return True, f"PDF report sent to {to_email}!"
...
# ============================================================================
# ACTIVE URGENCIES HELPER — single source of truth for filter settings
# ============================================================================
def get_active_urgencies() -> list:
    """
    Returns the list of urgency levels the user has enabled in
    Settings → Alert Filters. Defaults: CRITICAL + HIGH on, rest off.
    """
    active = []
...
# ============================================================================
# FDA DATA GENERATOR (embedded — no terminal needed)
# ============================================================================
def generate_fda_data_embedded():
    ...
    try:
            ...
            try:
                ...
                if r.status_code == 200:
                    for evt in r.json().get("results", []):
                        ...
                        base_static = (round(np.random.uniform(0.75, 0.95), 3) if "DEATH" in et else
            ...
            if r2.status_code == 200:
                for rec in r2.json().get("results", []):
                    ...
                    all_records.append({
                        ...
                        "gnn_risk": gnn, "xgb_risk": xgb, "total_risk": total,
                        "clinical_urgency": ("CRITICAL" if total>=0.75 else "HIGH" if total>=0.55
        ...
        if all_records:
            ...
            status_ph.error("❌ No FDA records could be fetched. Check your internet connection.")
            return False, 0
    except Exception as e:
        progress_bar_ph.empty()
        status_ph.error(f"❌ Error generating FDA data: {e}")
        return False, 0
...
def load_data() -> tuple:
    ...
    if not selected_dataset or selected_dataset not in all_datasets or \
            not all_datasets[selected_dataset]["exists"]:
...
def should_log_alert(urgency: str) -> bool:
    """
    Returns True only if the user has enabled logging for this urgency level
    in Settings → Alert Filters.
    """
    key_map = {
        "CRITICAL": "log_critical",
        "HIGH":     "log_high",
        "ELEVATED": "log_elevated",
        "MONITOR":  "log_monitor",
    }
...
def add_to_alert_log(device_name, device_id, urgency, score,
    ...
    # Persistent check — look at last entry for this device in history file
    try:
        history = load_alerts_history()
        # Only scan recent 200 entries for performance
        for entry in reversed(history[-200:]):
            if (str(entry.get("device_id")) == str(device_id) and
                ...
                if now_ts - prior_ts < dedupe_secs:
                    # Warm the cache to avoid repeated file reads
                    _RECENT_LOG_CACHE[cache_key] = prior_ts
                    return False
                break  # found the last entry, it's old enough — allow
    except Exception:
        pass
...
def send_bulk_email_alert(to_email: str, devices_df: pd.DataFrame) -> tuple:
    """
    Groups ALL urgency levels present in devices_df, not just CRITICAL/HIGH.
    The caller (run_auto_alerts) already pre-filters by active urgencies,
    so devices_df only contains rows the user wants alerted.
    """
    try:
        sender   = st.session_state.get("email_sender", EMAIL_SENDER)
        password = st.session_state.get("email_password", EMAIL_PASSWORD)
        ...
        # Build blocks for every urgency level that appears in the filtered df
        blocks = []
        ...
        body = (f"CLINICAL GUARDIAN — RISK ALERT REPORT\n{'='*40}\n"
                f"Time : {get_ist_now().strftime('%Y-%m-%d %H:%M:%S IST')}\n"
...
def send_bulk_sms_alert(to_number: str, devices_df: pd.DataFrame) -> tuple:
    """
    SMS summary reflects all active urgency levels, not just CRITICAL/HIGH.
    """
    try:
        urgency_counts = devices_df["urgency"].value_counts().to_dict()
        summary_parts  = [f"{cnt} {urg}" for urg, cnt in
        ...
        if ok:
            return True, f"Bulk SMS sent to {to_number}!"
        else:
            return False, msg
    except Exception as e:
...
# ============================================================================
# AUTO-ALERTS — uses get_active_urgencies() instead of hardcoded list
# ============================================================================
def run_auto_alerts(df: pd.DataFrame):
    ...
    # Build the filter from saved settings, not a hardcoded list
...
with st.sidebar:
    ...
    if df is not None:
        ...
        st.markdown(
            ...
            f"{'✅' if sms_ok else '❌'} SMS  ·  {'✅' if email_ok else '❌'} Email</div>",
            unsafe_allow_html=True,
        )

        # Show which urgency levels are being monitored
...
if df is None or len(df) == 0:
    st.error("❌ No dataset loaded. Use the **DATASET MANAGER** above to load a dataset.")
    st.stop()
...
if nav_choice == "📊 Dashboard":
    ...
    # Dataset Manager (collapsed by default)
        ...
        for i, (dataset_name, dataset_info) in enumerate(all_datasets.items()):
            ...
            with col2:
                if dataset_info["exists"]:
                    status_text = f"📄 {dataset_info['file']}<br/>📦 {dataset_info['size_mb']} MB · 🕐 {dataset_info['modified']}"
                else:
                    status_text = "⚠️ File not found"
                st.markdown(f"""
            ...
            if i < len(all_datasets) - 1:
                st.markdown("<hr style='border-color:rgba(255,255,255,.04);margin:4px 0;'>", unsafe_allow_html=True)

    st.markdown("---")

    # Auto-alert banner reflects actual urgency levels sent
    for label, count, ok, msg in st.session_state.get("auto_alert_results", []):
        if ok:
            st.success(f"✅ Auto-alert sent: {count} devices via {label}")
        else:
            st.warning(f"⚠️ Auto-alert attempt ({label}): {msg}")
    ...
    m5.metric("📊 AVG RISK", f"{disp['score'].mean():.3f}",
              f"{len(disp)} devices shown")
    ...
    search = st.text_input("🔍 Search devices…", placeholder="Name, ID, ward or type", key="dash_search")
    ...
    if len(page_df) > 0:
        ...
        for col_obj, lbl in zip(hh, ["Device","Ward","Type","Score","Status","Action"]):
            col_obj.markdown(f"<span style='font-size:10px;color:#475569;font-weight:600;'>{lbl}</span>", unsafe_allow_html=True)
        st.markdown("<hr style='border-color:rgba(255,255,255,.06);margin:3px 0 8px;'>", unsafe_allow_html=True)
        ...
        for _, row in page_df.iterrows():
            ...
            rc[1].markdown(wd)
            rc[2].markdown(f"<span style='font-size:11px;'>{tp[:14]}</span>", unsafe_allow_html=True)
            rc[3].markdown(f"<span style='color:{clr};font-weight:700;font-size:13px;'>{sc:.3f}</span>", unsafe_allow_html=True)
            rc[4].markdown(f"<span class='status-badge' style='background:{stat_clr}22;color:{stat_clr};'>{stat}</span>", unsafe_allow_html=True)
            with rc[5]:
                if st.button("View", key=f"view_{rid}", use_container_width=True):
                    st.session_state.sel = rid
                    st.rerun()

            st.markdown("<hr style='border-color:rgba(255,255,255,.03);margin:0;'>", unsafe_allow_html=True)
    ...
    with pg1:
        if st.button("⬅ Prev", key="pg_prev", disabled=st.session_state.page_num <= 1):
            ...
            st.rerun()
    with pg2:
        st.markdown(f"<div style='text-align:center;font-size:12px;color:#64748b;'>Page {st.session_state.page_num} / {total_pages}</div>", unsafe_allow_html=True)
    with pg3:
        ...
        if not m_row.empty:
            ...
            with dd1:
                ...
                <div style='background:#0f1520;border:1px solid rgba(255,255,255,.07);
                  ...
                  <div style='font-size:10px;color:#475569;margin-top:6px;'>⚠️ {rule['note']}</div>
                </div>""", unsafe_allow_html=True)
            ...
            with ac1:
                if st.button("🔌 Isolate", key="det_isolate", disabled=not rule["can_isolate"], use_container_width=True):
                    ...
                    st.cache_data.clear()
                    st.error(f"⚠️ Isolation triggered for {nm}")
            with ac2:
            ...
            with ac4:
                if st.button("📧 Alert", key="det_alert", use_container_width=True):
                    if st.session_state.get("email_enabled"):
                        ok, msg = send_email_alert(st.session_state.get("email_sender", ALERT_EMAIL_TO), dict(d), sc, urg)
                        st.success(msg) if ok else st.error(msg)
                    else:
                        st.warning("Email not configured. Go to ⚙️ Settings → Email tab.")
            with ac5:
                report = generate_device_report(d, sc, urg)
                st.download_button("📄 Report", report, f"report_{sel}_{get_ist_now().strftime('%Y%m%d')}.txt", "text/plain", key="det_report", use_container_width=True)
            with ac6:
            ...
            if not rule["can_isolate"]:
                st.info(f"ℹ️ Isolation disabled for this device type. {rule['note']}")
...
# ============================================================================
    ...
    with an1:
        ...
        wa.columns = ["ward", "avg"]
        wa["color"] = wa["avg"].apply(lambda s: URGENCY_COLOR.get("CRITICAL" if s>=0.75 else "HIGH" if s>=0.55 else "ELEVATED" if s>=0.30 else "MONITOR", "#888"))
        fig_a = go.Figure(go.Bar(x=wa["avg"], y=wa["ward"], orientation="h", marker_color=wa["color"], marker_line_width=0, text=[f"{v:.3f}" for v in wa["avg"]], textposition="outside"))
        fig_a.update_layout(plot_bgcolor="#0f1520", paper_bgcolor="#0f1520", font=dict(color="#94a3b8"), height=320, xaxis=dict(range=[0,1.2], gridcolor="rgba(255,255,255,.04)"), yaxis=dict(gridcolor="rgba(0,0,0,0)"), margin=dict(l=0, r=60, t=10, b=10), showlegend=False)
        st.plotly_chart(fig_a, use_container_width=True)

    with an2:
        st.markdown("#### 📉 Score Distribution")
        fig_h = go.Figure(go.Histogram(x=disp["score"], nbinsx=30, marker_color="#00d4ff", marker_line_width=0, hovertemplate="Score: %{x:.2f}<br>Count: %{y}<extra></extra>"))
        fig_h.update_layout(plot_bgcolor="#0f1520", paper_bgcolor="#0f1520", font=dict(color="#94a3b8"), height=320, xaxis=dict(title="Risk Score", gridcolor="rgba(255,255,255,.04)"), yaxis=dict(gridcolor="rgba(255,255,255,.04)"), margin=dict(l=0, r=0, t=10, b=40), showlegend=False)
        st.plotly_chart(fig_h, use_container_width=True)

    st.markdown("---")

    # Component breakdown scatter
    st.markdown("#### 🔬 Static vs Behaviour Risk")
    if "static" in disp.columns and "beh" in disp.columns:
        fig_sc = go.Figure(go.Scatter(
            x=disp["static"], y=disp["beh"],
            mode="markers",
            marker=dict(
                color=[URGENCY_COLOR.get(u, "#888") for u in disp["urgency"]],
                size=7, opacity=0.75,
            ),
    ...
    if len(disp) > 0:
        dt_analysis = disp.groupby("type")["score"].agg(["mean","max","min","count"]).round(3).sort_values("mean", ascending=False)
...
# ============================================================================
    ...
    # Show which urgency levels are currently being monitored
    active_urgencies = get_active_urgencies()
    if active_urgencies:
        pills = " · ".join(
        ...
        st.warning("⚠️ All urgency levels are disabled. No alerts will be logged. Go to ⚙️ Settings → Alert Filters to enable them.")

    # Manual alert trigger
    with st.expander("📤 Send Manual Alert", expanded=False):
        ma1, ma2 = st.columns(2)
        with ma1:
            alert_device = st.selectbox(
                "Select device to alert on",
        ...
        with ma2:
            if st.button("📧 Send Email Alert", key="manual_send_email"):
                ...
                if not row.empty and alert_email:
                    d   = row.iloc[0]
                    ok, msg = send_email_alert(alert_email, dict(d),
                                               float(d.get("score",0)),
                                               str(d.get("urgency","MONITOR")).upper())
                    if ok:
                        st.success(msg)
                    else:
                        st.error(msg)
                ...
                if not row.empty:
                    if alert_sms:
                        ...
                        body = (f"ClinGuard ALERT [{urg}]\n"
                                f"Device: {d.get('name','?')}\nWard: {d.get('ward','?')}\n"
                                f"Score: {float(d.get('score',0)):.3f}\nAction: {act[:80]}")
    ...
    if alerts_history:
        ...
        with tab_hist:
            display_cols = [c for c in ["timestamp","device_name","urgency","score","channel","status"] if c in alerts_df.columns]
            st.dataframe(alerts_df[display_cols].sort_values("timestamp", ascending=False).head(50), use_container_width=True)
            csv_alerts = export_to_csv(alerts_df)
            st.download_button("📥 Export Alerts CSV", csv_alerts, "alerts_history.csv", "text/csv", key="dl_alerts")

        with tab_stats:
            s1, s2, s3, s4 = st.columns(4)
            s1.metric("🔴 CRITICAL", int((alerts_df["urgency"]=="CRITICAL").sum()))
            s2.metric("🟠 HIGH",     int((alerts_df["urgency"]=="HIGH").sum()))
            s3.metric("🟡 ELEVATED", int((alerts_df.get("urgency","")=="ELEVATED").sum()))
            today_count = int((pd.to_datetime(alerts_df["timestamp"], format='ISO8601').dt.date == get_ist_now().date()).sum()) if len(alerts_df) > 0 else 0
    ...
    # Live feed respects active urgency filters
    ...
    if live_hr.empty:
        if active_urgencies:
            ...
            st.warning("⚠️ No alert filters are active.")
    else:
        for _, row in live_hr.iterrows():
            ...
            msg = (f"CVE risk {sv:.3f} — verify patch status."
                   if sv >= bv else
                   f"Behaviour anomaly {bv:.3f} — check network activity.")
            st.markdown(f"""
...
# ============================================================================
    ...
    inv_disp = inv_disp.sort_values(
        sort_map.get(inv_sort, "score"),
        ascending=(inv_sort != "Risk ↓"),
    )

    display_cols = [c for c in
                    ["name","id","type","ward","score","urgency","status","manufacturer","ip"]
...
# ============================================================================
            ...
            if st.button("💾 Save SMS Config", key="save_sms"):
                if all([new_sid, new_token, new_from, new_to]):
                    st.session_state.update({
                        ...
                        "twilio_from": new_from, "twilio_to": new_to,
                        "twilio_enabled": True,
                    })
    ...
    with tab_email:
        ...
        st.caption("Enable 2FA on your Google account → Security → App Passwords → "
    ...
    with tab_alerts:
        ...
        # Show current active state clearly before the checkboxes
        ...
        if active_now:
            ...
            st.warning("⚠️ No urgency levels are currently active — nothing will be alerted or logged.")
        ...
        if st.button("💾 Save Alert Settings", key="save_alerts"):
            ...
            # FIX: DO NOT pop auto_alerts_sent here - only REFRESH DATA button should clear it
            # This prevents auto-alerts from re-firing on settings save
    ...
    with tab_data:
        ...
        with dm1:
            csv_data = export_to_csv(disp)
            st.download_button(
                "📥 Export Current View as CSV", csv_data,
        ...
        if FPDF_AVAILABLE and len(disp[disp["urgency"] == "CRITICAL"]) > 0:
            if st.button("📋 Generate Critical Devices PDF", key="gen_pdf", use_container_width=True):
                with st.spinner("Generating PDF..."):
                    pdf_buffer = generate_ward_wise_critical_report(disp)
                    if pdf_buffer:
                        st.download_button(
            ...
            st.info("ℹ️ PDF library not available. Install: pip install fpdf2")
        else:
            st.info("ℹ️ No CRITICAL devices to report on")

        st.markdown("---")

        # Bulk Email PDF Feature
        ...
        if st.button("📧 Send PDF Email", key="send_pdf_email", use_container_width=True):
            if pdf_email:
                with st.spinner("Sending PDF email..."):
                    ok, msg = send_bulk_email_alert_PDF(pdf_email, disp)
                    st.success(msg) if ok else st.error(msg)
            else:
                st.warning("Enter an email address")

        st.markdown("---")
        st.markdown("#### ℹ️ About Clinical Guardian v8.0")
        st.markdown(f"""
        ...
        **Time:** {get_ist_now().strftime('%d %b %Y %H:%M:%S IST')}
        ...
        ---
        ⚠️ *Decision support tool only — not a substitute for clinical judgment.*
        """)
...
st.caption(
    ...
    f"GNN + XGBoost Fusion  ·  For Hospital Use Only"
)
